...
study_program::st_pr::st_pr(std::size_t _nstudents, std::size_t _ngroup) : nstudents(_nstudents), ngroup(_ngroup) {
...
std::ostream &study_program::operator<<(std::ostream &stream, study_program::st_pr &st) {
    ...
    /* I/O flow */
    stream << "Current number of students: " << st.curr_nstudents << std::endl <<
            "Total number of students: " << st.nstudents << std::endl <<
            "Percentage of abandoned students: " << (static_cast<double>(cnt) / static_cast<double>(st.nstudents)) * 100 << "%" << std::endl <<
            "Number of groups: " << st.ngroup;
...
std::istream &study_program::operator>>(std::istream &stream, study_program::st_pr &st) {
    ...
    stream >> st.nstudents;
    std::cout << "Type the number of groups: ";
    stream >> st.ngroup;
