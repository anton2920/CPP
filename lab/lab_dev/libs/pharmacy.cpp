...
void pharmacy::setPrice(size_t price) {
...
pharmacy::date pharmacy::date::operator+(std::time_t days) const {
    ...
    std::time_t days1 = this->day + this->month * 12 + this->year * 365;
    std::time_t days2 = days;
    std::time_t res = days1 + days2;
    date tmp;

    /* Main part */
    tmp.year = res / 365;
    tmp.month = (res % 365) % 12;
    tmp.day = (res % 365 % 12) % 30;
...
pharmacy::date pharmacy::date::operator+(const pharmacy::date &other) const {
    ...
    /* Initializing variables */
    std::time_t days1 = this->day + this->month * 12 + this->year * 365;
    std::time_t days2 = other.day + other.month * 12 + other.year * 365;
    std::time_t res = days1 + days2;
    date tmp;
